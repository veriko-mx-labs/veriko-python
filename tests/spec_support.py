...
https://docs.veriko.mx/openapi.yaml, que ya está filtrado por visibilidad. Nunca
se usa el bundle interno de la aplicación: trae las rutas `/admin/*`.
"""
...
def security(operation: dict[str, Any]) -> list[dict[str, Any]]:
    """Los esquemas de autenticación alternativos que acepta la operación."""
...
def accepts_api_key(operation: dict[str, Any]) -> bool:
    """`True` si la operación se usa de máquina a máquina.

    Es la que acepta la clave de API, o la que es pública y no pide autenticación.
    Una operación que sólo acepta la cookie de sesión es de la interfaz.
    """
